...
for (file in tsv_files) {
  ...
  for (i in 1:(length(specific_harmonic_tab$chord) - 1)) {
    ...
    if (current_chord != "" && next_chord != "") {
      if (specific_harmonic_tab$localkey_is_minor[i] == 1 && specific_harmonic_tab$localkey_is_minor[i + 1] == 1) { # Filter
...
# # Transposition
# transpo_transition_matrix <- t(transition_matrix)
...
# Heatmap erstellen
library(ggplot2)
ggplot(data = transition_df, aes(y = Var1, x = Var2, fill = Freq)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "#B7352D") +
  theme_minimal() + 
  theme(
...
ggsave("transition_probab_minor-480dpi-final-2-5.png", plot = last_plot(), width = 8, height = 8, dpi = 480)
