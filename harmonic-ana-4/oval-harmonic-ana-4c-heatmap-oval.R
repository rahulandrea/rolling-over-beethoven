...
setwd("~/Library/CloudStorage/OneDrive-SBL/32-Maturaarbeit/Analyse/harmonic-ana-4")
...
ggplot(data = transition_df, aes(x = Var2, y = Var1, fill = Freq)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "#627313") +
  theme_minimal() + 
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
  ) +
  coord_fixed(ratio=1) +
  labs(title = "transition probab A to B",
       fill = "probability",
       x = "B",
       y = "A") +
  geom_text(
...
# ggsave("transition_probab-480dpi.png", plot = last_plot(), width = 8, height = 8, dpi = 480)
