# harmonic-ana-5x-final-tonikalisierung: Mittlere Entropie zufälliger Stichproben (Häufigkeitsgewichtet)
# verglichen mit der mittleren Entropie der Akkorde in Tonikalisierung
...
# Ton. bedeutet: Eintrag in oval_harmonic_tab$relativeroot
# Prüfe vers. mögliche Einträge in Zeile: oval_harmonic_tab$relativeroot
# > print(unique(oval_harmonic_tab$relativeroot))
# dh. Einträge "" sollen ignoriert werden

# Nimm harmonic-ana-5c-suspend un passe an für Ton. (A) 
# und nimm harmonic-ana-5d-samples (B) und Plotte beide in dieselbe Grafik (C)
...
for (file in tsv_files) {
  ...
  for (i in 1:(length(B_specific_harmonic_tab$chord) - 1)) {
    ...
    if (current_chord != "" && next_chord != "") {
      ...
      if (B_specific_harmonic_tab$localkey_is_minor[i] == 1 && B_specific_harmonic_tab$localkey_is_minor[i + 1] == 1) {
...
# (A.1.a) Matrix mit Übergangshäufigkeiten (harmonic-ana-5c (aber für Umkehrungen))
...
for (file in tsv_files) {
  ...
  for (i in 1:(length(A_specific_harmonic_tab$chord) - 1)) {
    ...
    if (current_chord != "" && next_chord != "") {
      # Stellt Bed. an Dur- bzw. Moll-Abschnitt 
      if (A_specific_harmonic_tab$localkey_is_minor[i] == 1 && A_specific_harmonic_tab$localkey_is_minor[i + 1] == 1) {
        # Stellt Bed., dass current_chord in Tonikalisierung liegt
        if (!is.na(A_specific_harmonic_tab$relativeroot[i]) && A_specific_harmonic_tab$relativeroot[i] != "") {
          print(A_specific_harmonic_tab$relativeroot[i])
          # Fordert, dass current_chord und next_chord in der Matrix sind 
          if (current_chord %in% all_chords && next_chord %in% all_chords) {
...
write.table(B_proben_entr_df, "B-ton-min.tsv", sep = "\t", row.names = FALSE)
...
print("Mittel der Entropien der Akkorde in Moll-Abschnitten in Tonikalisierung:")
print(A_entropy_mean)
print("Mittel der Entropien der Akkorde in Moll-Abschnitten der Stichproben:")
print(mean(B_proben_entr_df$Mittl_NormEntropie))

# Speichere das Diagramm als PNG-Datei
# ggsave("entropy_final_ton-480dpi-minor.png", plot = last_plot(), width = 8, height = 8, dpi = 480)
