# harmonic-ana-5x-final-inversion: Mittlere Entropie zufälliger Stichproben (Häufigkeitsgewichtet)
# verglichen mit der mittleren Entropie der Akkorde mit Umkehrung
...
# Umkehrung bedeutet:
# Dreiklang: oval_harmonic_tab$figbass == 6 && == 64
# Septakkord: oval_harmonic_tab$figbass == 65 && == 73 && ==2
# Achtung! oval_harmonic_tab$figbass == 7 bed. Grundseptakkord
# Prüfe vers. mögliche Einträge in Zeile: oval_harmonic_tab$figbass
# > print(unique(oval_harmonic_tab$figbass))
# [Ausgabe:] NA 65  6  7 64  2 43


# Nimm harmonic-ana-5c-suspend un passe an für inversion (A) 
  ...
  for (i in 1:(length(A_specific_harmonic_tab$chord) - 1)) {
    ...
    if (current_chord != "" && next_chord != "") {
      ...
      if (A_specific_harmonic_tab$localkey_is_minor[i] == 1 && A_specific_harmonic_tab$localkey_is_minor[i + 1] == 1) {
        # Stellt Bed., dass current_chord eine Umkehrung hat # Also Wert in figbass ungl. 7
        if (!is.na(A_specific_harmonic_tab$figbass[i]) && A_specific_harmonic_tab$figbass[i] != "" && A_specific_harmonic_tab$figbass[i] != 0 && A_specific_harmonic_tab$figbass[i] != 7) {
          # print(A_specific_harmonic_tab$figbass[i])
          # Fordert, dass current_chord und next_chord in der Matrix sind 
          if (current_chord %in% all_chords && next_chord %in% all_chords) {
...
write.table(B_proben_entr_df, "B-inv-min.tsv", sep = "\t", row.names = FALSE)
...
print("Mittel der Entropien der Akkorde mit Umkehrung:")
print(A_entropy_mean)

# Speichere das Diagramm als PNG-Datei
# ggsave("entropy_final_inversion-480dpi-minor.png", plot = last_plot(), width = 8, height = 8, dpi = 480)
