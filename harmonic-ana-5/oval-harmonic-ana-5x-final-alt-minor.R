# harmonic-ana-5x-final-alteration: Mittlere Entropie zufälliger Stichproben (Häufigkeitsgewichtet)
# verglichen mit der mittleren Entropie der Akkorde mit Alteration
...
# Alteration bedeutet: oval_harmonic_tab$chord beginnt mit "#" oder "b"

# Nimm harmonic-ana-5c-suspend un passe an für Alteration (A) 
  ...
  for (i in 1:(length(A_specific_harmonic_tab$chord) - 1)) {
    ...
    if (current_chord != "" && next_chord != "") {
      ...
      if (A_specific_harmonic_tab$localkey_is_minor[i] == 1 && A_specific_harmonic_tab$localkey_is_minor[i + 1] == 1) {
        # Stellt Bed., dass current_chord mit Alteration
...
write.table(B_proben_entr_df, "B-alt-maj.tsv", sep = "\t", row.names = FALSE)
...
print("Mittel der Entropien der Akkorde in Dur-Abschnitten in Alteration:")
print(A_entropy_mean)
print("Mittel der Entropien der Akkorde in Dur-Abschnitten der Stichproben:")
print(mean(B_proben_entr_df$Mittl_NormEntropie))

# Speichere das Diagramm als PNG-Datei
# ggsave("entropy_final_ton-480dpi-major.png", plot = last_plot(), width = 8, height = 8, dpi = 480)
