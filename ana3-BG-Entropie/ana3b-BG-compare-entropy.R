# ana3-BG-Entropie: ana3b-BG-compare-entropy.R
...
tsv_files_base <- list.files(path = "Daten", pattern = "LVB-S(0[0-9]|10|19|20)-M[0-9].tsv")
tsv_files_dev1 <- list.files(path = "Daten", pattern = "LVB-S3[0-9]-M[0-9].tsv")
...
# (BASE)
  ...
  all_chords_base <- unique(chord_freq_df_base$chord)
...
# (DEV1)
  ...
  # (B) 
    ...
    all_chords_dev1 <- unique(chord_freq_df_dev1$chord)
...
# (C)
  ...
  chord_freq_df$absolute_frequency_base <- ifelse(chord_freq_df$chord %in% chord_freq_df_base$chord, chord_freq_df_base$absolute_frequency[match(chord_freq_df$chord, chord_freq_df_base$chord)], 0)
  ...
  chord_freq_df$absolute_frequency_dev1 <- ifelse(chord_freq_df$chord %in% chord_freq_df_dev1$chord, chord_freq_df_dev1$absolute_frequency[match(chord_freq_df$chord, chord_freq_df_dev1$chord)], 0)
...
# BASE (1)
  ...
  # (1a) Umfassende Matrix aller Akkordübergänge
    ...
    # Matrix erstellen
    base_transition_matrix <- matrix(0, nrow = length(all_chords_base), ncol = length(all_chords_base), dimnames = list(all_chords_base, all_chords_base))
      ...
      for (i in 1:(length(specific_harmonic_tab$chord) - 1)) {
        ...
        if (current_chord != "" && next_chord != "") {
          if (specific_harmonic_tab$localkey_is_minor[i] == tongeschlecht && specific_harmonic_tab$localkey_is_minor[i + 1] == tongeschlecht) { # Filter
            if (current_chord %in% all_chords_base && next_chord %in% all_chords_base) {
              base_transition_matrix[current_chord, next_chord] <- base_transition_matrix[current_chord, next_chord] + 1
            }
          }
        }
      }
    ...
    for (i in 1:nrow(base_transition_matrix)) {
      for (j in 1:ncol(base_transition_matrix)) {
        base_transition_matrix[i,j] <- base_transition_matrix[i,j] / chord_freq_df[chord_freq_df$chord == rownames(base_transition_matrix)[i], "absolute_frequency_base"]
      }  
    }
  ...
  # (1b) Berechnung Entropie aller Akkorde und Normierung
    ...
    relevant_chords <- rownames(base_transition_matrix)
    entropy_df_base <- data.frame(
      entropy = numeric(length(relevant_chords))
    )
    
    rownames(entropy_df_base) <- relevant_chords
    ...
    for (c in relevant_chords){
      ...
      for (j in 1:ncol(base_transition_matrix)) {
        transition_freq <- base_transition_matrix[start_chord, j] # rel. Häufigkeit d. Übergangs
    ...
    for (g in 1:length(entropy_df_base$norm_entropy)){
      entropy_df_base$rel_freq[g] <- chord_freq_df$relative_frequency_base[g]
    }
    
    expected_entropy_base <- sum(entropy_df_base$norm_entropy * entropy_df_base$rel_freq)

    entropy_df_base$chord <- rownames(entropy_df_base)
...
# DEV1 (2)
  ...
  # (2a) Umfassende Matrix aller Akkordübergänge
    ...
    # Matrix erstellen
    dev1_transition_matrix <- matrix(0, nrow = length(all_chords_dev1), ncol = length(all_chords_dev1), dimnames = list(all_chords_dev1, all_chords_dev1))
    ...
    for (file in tsv_files_dev1) {
      ...
      for (i in 1:(length(specific_harmonic_tab$chord) - 1)) {
        ...
        if (current_chord != "" && next_chord != "") {
          if (specific_harmonic_tab$localkey_is_minor[i] == tongeschlecht && specific_harmonic_tab$localkey_is_minor[i + 1] == tongeschlecht) { # Filter
            if (current_chord %in% all_chords_dev1 && next_chord %in% all_chords_dev1) {
              dev1_transition_matrix[current_chord, next_chord] <- dev1_transition_matrix[current_chord, next_chord] + 1
            }
          }
        }
      }
    ...
    for (i in 1:nrow(dev1_transition_matrix)) {
      for (j in 1:ncol(dev1_transition_matrix)) {
        dev1_transition_matrix[i,j] <- dev1_transition_matrix[i,j] / chord_freq_df[chord_freq_df$chord == rownames(dev1_transition_matrix)[i], "absolute_frequency_dev1"]
      }  
    }
  ...
  # (2b) Berechnung Entropie aller Akkorde und Normierung
    ...
    # Dataframe erstellen um für jeden Akkord mit seiner Entropie einzufügen
    relevant_chords <- rownames(dev1_transition_matrix)
    entropy_df_dev1 <- data.frame(
      entropy = numeric(length(relevant_chords))
    )
    
    rownames(entropy_df_dev1) <- relevant_chords
    ...
    for (c in relevant_chords){
      ...
      for (j in 1:ncol(dev1_transition_matrix)) {
        transition_freq <- dev1_transition_matrix[start_chord, j] # rel. Häufigkeit d. Übergangs
    ...
    for (h in 1:length(entropy_df_dev1$norm_entropy)){
      entropy_df_dev1$rel_freq[h] <- chord_freq_df$relative_frequency_dev1[h]
    }
    
    expected_entropy_dev1 <- sum(entropy_df_dev1$norm_entropy * entropy_df_dev1$rel_freq)
    
    entropy_df_dev1$chord <- rownames(entropy_df_dev1)
...
ggsave("ana3b-BG-compare-entropy_LVBS-early-late-minor-480dpi.png", plot = last_plot(), path = "Ergebnisse/In-Skript", width = 8, height = 5, dpi = 480)
