...
# in diesem Fall gefiltert nach "localkey_is_minor == 0" dh. lokalen Dur Akkorden
...
# Ggf. Filter: Nur Einträge wo "globalkey_is_minor" == 0
...
ggplot(chord_freq_df, aes(x = rank, y = relative_frequency)) +
  geom_point(shape = 1, size = 1, color = "cornflowerblue") +
  scale_y_continuous(trans = "log10") +
  scale_x_continuous(trans = "log10") +
  #  scale_y_continuous(labels = scales::label_number()) +
  labs(x = "chords_rank", y = "rel_freq", title = "rank vs frequency (major chords)") +
  theme_minimal()
...
ggsave("rank_vs_freq_major2-1200dpi.png", plot = last_plot(), width = 8, height = 6, dpi = 1200)